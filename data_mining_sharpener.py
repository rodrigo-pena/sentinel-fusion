...
@click.command()
@click.option('--sentinel_2_reflectance', required=True,
              type=click.Path(dir_okay=False, exists=True))
@click.option('--sentinel_3_lst', required=True,
              type=click.Path(dir_okay=False, exists=True))
@click.option('--high_res_dem', required=True,
              type=click.Path(dir_okay=False, exists=True))
@click.option('--high_res_geom', required=True,
              type=click.Path(dir_okay=False, exists=True))
@click.option('--lst_quality_mask', required=True,
              type=click.Path(dir_okay=False, exists=True))
@click.option('--date_time_utc', required=True,
...
def main(sentinel_2_reflectance, sentinel_3_lst, high_res_dem,
         high_res_geom, lst_quality_mask, date_time_utc,
    ...
    cos_theta = incidence_angle_tilted(lat, lon, doy, ftime,
                                       stdlon=0, A_ZS=aspect, slope=slope)
    ...
    fp = gu.merge_raster_layers([temp_refl_file,
                                 temp_dem_file,
                                 temp_cos_theta_file],
...
def declination_angle(doy):
    ...
    '''
    declination = np.radians(23.45) \
        * np.sin((2.0 * np.pi * doy / 365.0) - 1.39)
...
def incidence_angle_tilted(lat, lon, doy, ftime, stdlon=0, A_ZS=0, slope=0):
    ...
    cos_theta_i = (
        np.sin(delta) * np.sin(lat) * np.cos(slope)
