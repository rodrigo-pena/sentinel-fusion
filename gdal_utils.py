...
# If the conda environment is not active, make sure to set the env_variables
# needed for gdal
# import os
...
def raster_data(raster, bands=1, rect=None):
    ...
    def _read_band(fid, band, rect):
        if rect:
            return fid.GetRasterBand(band).ReadAsArray(rect.x,
                                                       rect.y,
                                                       rect.width,
                                                       rect.height)
        else:
...
def merge_raster_layers(input_list, output_filename, separate=False):
    ...
    for input_file in input_list:
        ...
        # GDAL Build VRT cannot stack multiple multi-band images, so they have
        # to be split into multiple single-band images first.
        if bands > 1:
            for band in range(1, bands + 1):
                tmp_filename = tempfile.mkstemp(suffix="_"+str(band)+".vrt")[1]
                gdal.BuildVRT(tmp_filename, [input_file], bandList=[band])
                merge_list.append(tmp_filename)
        else:
